...
class AutoTagsManager:
    ...
    def __init__(self):
        self.tiratori_file = os.path.join('data', 'Tiratori', 'Tiratori.json')
        self.titolarita_file = os.path.join('data', 'Titolarita', 'Titolarita.json')
        self.player_notes = PlayerNotesManager()

    def load_tiratori_data(self) -> List[Dict]:
        """Carica i dati dei tiratori dal file JSON"""
        if not os.path.exists(self.tiratori_file):
    ...
    def load_titolarita_data(self) -> List[Dict]:
        ...
        if not os.path.exists(self.titolarita_file):
