...
class OptimizedPriceCalculator:
    ...
    def _has_historical_stats(self, player_row):
        """Restituisce True se il giocatore ha almeno una statistica storica non-zero.

        Un giocatore puo' avere seasons_count > 0 ma non aver mai disputato
        partite: in quel caso tutte le statistiche storiche possono risultare
        pari a zero. Per il pricing viene quindi considerato equivalente a un
        giocatore senza storico utilizzabile.
        """
        stat_names = (
            'Pv', 'Mv', 'Fm', 'Gf', 'Gs', 'Rp', 'Rc', 'R+', 'R-',
            'Ass', 'Amm', 'Esp', 'Au',
        )

        for stat_name in stat_names:
            if self._get_stat_value(player_row, stat_name) != 0.0:
                return True
        return False
    ...
    def calculate_price_percentage(self, player_id, budget=500):
        ...
        Per i giocatori completamente privi di dati storici, oppure con uno
        storico presente ma con tutte le statistiche a zero (es. giocatori
        presenti in rosa senza aver mai giocato), usa la quotazione FVM del
        listone come fallback diretto in crediti.
        """
        ...
        # Fallback FVM: si applica ai giocatori senza dati storici utilizzabili.
        # Questo include sia seasons_count == 0 sia i casi in cui esiste uno
        # storico nominale ma tutte le statistiche sono pari a zero.
        try:
            seasons_count = int(player_row.get('seasons_count', 0))
        except (TypeError, ValueError):
            seasons_count = 0

        has_historical_stats = self._has_historical_stats(player_row)

        if seasons_count == 0 or not has_historical_stats:
            fvm = player_row.get('FVM')
            if fvm is not None and not pd.isna(fvm):
                try:
                    if isinstance(fvm, str):
                        fvm = fvm.replace(',', '.').strip()
                    fvm = float(fvm)
                except (TypeError, ValueError):
                    fvm = 0.0

                if fvm > 0:
                    percentage = (fvm / budget * 100) if budget else 0.0
                    return {
                        'percentage': round(percentage, 1),
                        'credits': int(round(fvm)),
                        'budget': budget,
                        'breakdown': {
                            'source': 'FVM',
                            'fvm': round(fvm, 2),
                            'reason': (
                                'Nessun dato statistico storico disponibile'
                                if seasons_count == 0
                                else 'Storico presente ma tutte le statistiche sono a zero'
                            )
                        }
                    }
        ...
        # Applica fixture difficulty modifier
        squadra = player_row.get('Squadra', '')
        fixture_multiplier = self._calculate_fixture_difficulty_modifier(squadra, ruolo)
        percentage *= fixture_multiplier
    ...
    def _calculate_fixture_difficulty_modifier(self, team: str, role: str) -> float:
        """
        Calcola modificatore basato su fixture difficulty stagionale
        ...
        Returns:
            float: Moltiplicatore 0.95-1.05 (±5%)
        """
        try:
            calculator = get_fixture_calculator()
            summary = calculator.get_fixture_summary(team, matchdays=38)

            # Avg difficulty: 1-10 (1=facile, 10=difficile)
            avg_difficulty = summary.get('avg_difficulty', 5.0)

            # Normalizza: 5.0 = neutro (1.0x)
            # Formula: 1.0 + (5.0 - avg_difficulty) * 0.01
            modifier = 1.0 + (5.0 - avg_difficulty) * 0.01
            ...
            # Fallback neutro se calendario non disponibile
            return 1.0
