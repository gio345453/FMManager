...
from src.data.defense_modifier import calculate_defense_contribution_price
...
class PriceCalculator:
    ...
    def _calculate_single_price(self, player_id, budget_total):
        ...
        # 13. Defense modifier - MOLTIPLICATIVO per P e D
        defense_multiplier = calculate_defense_contribution_price(
            {
                'mv_weighted': player_row.get('Mv_weighted'),
                'gs_weighted': player_row.get('Gs_weighted'),
                'pv_weighted': player_row.get('Pv_weighted')
            },
            role
        )

        # 14. Fixture difficulty modifier - MOLTIPLICATIVO (calendario favorevole/sfavorevole)
        fixture_multiplier = self._calculate_fixture_difficulty_modifier(team, role)
        ...
        final_percentage = ((base_score * team_coeff * team_performance * presence_mod *
                           ...
                           scarcity_bonus * fm_bonus * titolarita_bonus * defense_multiplier *
                           fixture_multiplier) +
    ...
    def _calculate_fixture_difficulty_modifier(self, team: str, role: str) -> float:
        ...
        Calendario facile -> prezzo più alto (giocatori performano meglio)
        Calendario difficile -> prezzo più basso
