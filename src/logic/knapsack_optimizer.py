...
from src.data.defense_modifier import (
    calculate_defense_contribution_overall,
    estimate_seasonal_defense_bonus
)
...
class KnapsackOptimizer:
    ...
    def optimize_positions(self, empty_positions: List[int], position_roles: List[str],
                          ...
                          custom_credits: Dict[int, float] = None,
                          blacklisted_player_ids: List[int] = None) -> Dict[int, Dict]:
        """
    ...
    def _optimize_unified_cpsat(self, classification, selected_players, value_weights,
        ...
        Args:
            ...
            budget_per_role: Budget % per ruolo
            blacklisted_player_ids: IDs giocatori da escludere
        ...
        # Rilassa vincoli minimi se ci sono blacklist attive
        has_blacklist = bool(blacklisted_teams) or bool(blacklisted_player_ids)
        ...
        # Soglie minime presenze - RIDOTTE se ci sono molti giocatori bloccati
        # Con pochi slot da riempire, i requisiti di presenze devono essere più flessibili
        total_slots = sum(len(classification[r]) for r in ['P', 'D', 'C', 'A'])
        slots_ratio = total_slots / 25.0  # Percentuale slot da riempire

        # Riduci requisiti se devi riempire meno del 30% dei slot (< 8 giocatori)
        if slots_ratio < 0.3:
            MIN_PRESENZE = {
                'P': 3,   # Ridotto da 5
                'D': 15,  # Ridotto da 24
            ...
            print(f"[CP-SAT] Pochi slot da riempire ({total_slots}/25), requisiti presenze RIDOTTI: P=3, D/C/A=15")
        else:
            MIN_PRESENZE = {
                'P': 5,
                'D': 24,
            ...
            print(f"[CP-SAT] Requisiti presenze STANDARD: P=5, D/C/A=24")
        ...
        if slots_without_candidates:
            error_msg = f"Impossibile completare la rosa: {len(slots_without_candidates)} slot senza candidati.\n"
            error_msg += "Dettagli:\n"
            for slot in slots_without_candidates:
                error_msg += f"  • Ruolo {slot['role']}, Posizione {slot['pos']}, Fascia '{slot['tier']}', Budget {slot['budget']:.1f} crediti\n"
            error_msg += "\nPossibili cause:\n"
            error_msg += "  • Requisiti presenze troppo alti (D/C/A: 24 presenze, P: 5 presenze)\n"
            error_msg += "  • Budget insufficiente per il ruolo\n"
            error_msg += "  • Troppi giocatori in blacklist\n"
            error_msg += "\nSoluzioni:\n"
            error_msg += "  • Ridurre il numero di giocatori bloccati\n"
            error_msg += "  • Aumentare il budget per questi ruoli\n"
            error_msg += "  • Rimuovere vincoli di blacklist\n"
        ...
        # Rilassa ROLE_OVERFLOW se ci sono custom credits (prezzi modificati dall'utente)
        # per dare più flessibilità al solver
        if custom_credits:
            print(f"[CP-SAT] Custom credits detected: {custom_credits}, using ROLE_OVERFLOW=100")
            ROLE_OVERFLOW = {
                'P': 100,  # Molto più flessibile con prezzi custom
                'D': 100,
                'C': 100,
                'A': 100,
            }
        else:
            print(f"[CP-SAT] No custom credits, using standard ROLE_OVERFLOW")
            ROLE_OVERFLOW = {
                'P': 30,
                'D': 30,
                'C': 20,
        ...
        for role in ['P', 'D', 'C', 'A']:
            ...
            if role_slots:
                ...
                # Rilassa il vincolo se ci sono blacklist attive per aumentare flessibilità
                if role_remaining_budget >= 1.0 and not has_blacklist:
                    model.Add(
    ...
    def _get_candidates_for_slot(self, available, pos, tier_budget, weight, value_weights, custom_credits):
        ...
        for idx, player_row in available.iterrows():
            ...
            # Bonus defense modifier per P e D
            base_role = player_row.get('R', '')[0] if player_row.get('R') else ''
            defense_bonus = 0
            if base_role in ['P', 'D']:
                # Stima bonus stagionale e convertilo in score
                seasonal_impact = estimate_seasonal_defense_bonus(
                    {
                        'mv_weighted': mv,
                        'gs_weighted': self._to_float(player_row.get('Gs_weighted', 0), 0),
                        'pv_weighted': pv
                    },
                    base_role
                )
                # Bonus totale stagionale / 10 per convertirlo in score comparabile
                defense_bonus = seasonal_impact['bonus_totale'] / 10

            # Applica peso fascia per rendimenti decrescenti
            weighted_score = (base_score + elite_bonus + defense_bonus) * weight
    ...
    def _knapsack_dynamic_programming(self, role, tier_slots, selected_players, value_weights,
        ...
        for slot_info in tier_slots:
            ...
            for idx, player_row in available.iterrows():
                ...
                weighted_score = (base_score + defense_bonus) * weight
