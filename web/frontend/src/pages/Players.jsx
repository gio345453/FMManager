...
import { readMediaPreference, writeMediaPreference } from '../utils/playerMedia';
...
function Players() {
  ...
  useEffect(() => {
    if (!globalLoading && allPlayers.length > 0) {
      applyFilters();
    }
  }, [allPlayers, filters, globalLoading]);
  ...
  const applyFilters = () => {
    ...
    if (filters.favorite) filtered = filtered.filter((p) => p.is_favorite);
    ...
    <div className="min-h-screen w-full bg-[#0B0E14] text-slate-100">
      <div className="mx-auto max-w-[1800px] px-4 py-5 sm:px-6 lg:px-8 lg:py-7">
        <header className="mb-6 flex flex-col gap-4 border-b border-slate-800/70 pb-5 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <div className="flex items-center gap-3">
              <h1 className="text-2xl font-semibold tracking-tight text-slate-50 sm:text-3xl">Giocatori</h1>
            ...
            <p className="mt-1 text-sm text-slate-500">
              Ricerca, filtra e confronta tutti i giocatori disponibili.
            </p>
          </div>
        ...
        <section className="mb-5 overflow-hidden rounded-2xl border border-slate-800/80 bg-[#0F172A]/85 shadow-[0_10px_40px_rgba(0,0,0,0.18)]">
          ...
          <div className="grid gap-4 p-4 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 sm:p-5">
            ...
            <label className="flex min-h-10 cursor-pointer items-center gap-3 rounded-xl border border-slate-800 bg-slate-950/40 px-3.5 py-2 text-sm text-slate-300 transition-colors hover:border-slate-700 hover:bg-slate-900 sm:col-span-2 lg:col-span-1">
              <input
                type="checkbox"
                checked={filters.favorite}
                onChange={(e) => handleFilterChange('favorite', e.target.checked)}
                className="h-4 w-4 rounded border-slate-600 bg-slate-900 text-sky-400 accent-sky-400"
              />
              <Star className="h-4 w-4 text-amber-400" aria-hidden="true" />
              <span className="font-medium">Solo preferiti</span>
            </label>
          </div>
        </section>
        ...
        {displayedPlayers.length === 0 ? (
          <div className="rounded-2xl border border-slate-800/80 bg-[#0F172A]/70 p-6 sm:p-10">
            <EmptyState
              ...
              description="Prova a modificare i filtri o la ricerca per trovare altri giocatori."
              action={{ label: 'Reset filtri', onClick: resetFilters, variant: 'outline' }}
            />
          </div>
        ) : (
          <ResponsiveTable
            caption="Elenco giocatori filtrati e ordinati"
            mobileContent={
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex items-center gap-3 min-w-0">
                        {showMedia && <PlayerAvatar playerId={player.id} size="medium" />}
                        <div className="min-w-0">
                          <div className="truncate text-[15px] font-semibold text-slate-100">{player.nome}</div>
                          <div className="mt-1 flex items-center gap-2 text-xs text-slate-500">
                            {showMedia && <TeamLogo teamName={player.squadra} size={20} />}
                    ...
                    >
                      ...
                      <td className="px-3 py-3.5">
                        <div className="flex items-center gap-2 text-sm text-slate-400">
                          {showMedia && <TeamLogo teamName={player.squadra} size={24} />}
                          <span>{player.squadra}</span>
                        </div>
                      </td>
                      <td className="px-3 py-3.5">
